<?php
// Get criteria and questions data for supervisor evaluator_type
include('../config/database.php');

// Get criteria for dropdown
$criteriaOptions = [];

$criteriaQuery = "SELECT id, name FROM criteria WHERE evaluator_type = 'supervisor' ORDER BY name";
...
$questionsQuery = "SELECT q.id, q.question_text, q.criteria_id, c.name 
                   ...
                   WHERE c.evaluator_type = 'supervisor' AND COALESCE(q.deleted,0) = 0
...
// Map supervisor evaluations to 'peer' period type by default (supervisor evaluates teachers)
...
<div class="w-full h-auto p-4 overflow-y-hidden">
    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Manage Supervisor Questionnaire</h1>
        <p class="text-gray-600 mt-1">Create and manage evaluation questions for supervisors evaluating teachers</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <form action="../actions/AddQuestion.php" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative flex items-center">
                        ...
                        <button type="button" onclick="document.getElementById('addCriteriaModalSupervisor').style.display='block'" class="absolute right-2 top-1/2 -translate-y-1/2 px-2 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-xs" title="Add Criteria">+
                        </button>
                    </div>
        ...
        <?php include('modal/add_criteria_supervisor_modal.php') ?>
