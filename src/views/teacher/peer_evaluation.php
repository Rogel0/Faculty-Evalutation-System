...
<div class="p-2">
    <div class="overflow-x-auto">
        ...
        $teacher_id = $_SESSION['userID'] ?? null;
        if (!$teacher_id) {
            echo '<div class="p-8 text-center text-red-600">Not logged in.</div>';
            exit;
        }

        // Get selected colleague and details
        ...
        $teacherRes->execute();
        $teacherRes->store_result();
        if ($teacherRes->num_rows === 0) {
            echo '<div class="p-8 text-center text-red-600">Teacher not found.</div>';
            exit;
        }
        $teacherRes->bind_result($firstname, $lastname, $department);
        $teacherRes->fetch();
        $teacherRes->close();

        // Get colleagues in same department
        $colleagues = [];
        $sql = "SELECT id, firstname, lastname, department, position
        FROM users 
        WHERE user_type = 'teacher' AND department = ? AND id != ?
        ...
        // Check if teacher has already evaluated this colleague
        ...
        // Get evaluation questions for teachers
        $questionsData = [];
        if ($selected_colleague) {
        ...
        <div class="w-full h-auto p-0 md:p-4 overflow-y-hidden">
            ...
            <div class="mb-4 md:mb-6 px-2 md:px-0">
                <h1 class="text-xl md:text-2xl font-bold text-gray-900">Peer Evaluation</h1>
                <p class="text-gray-600 mt-1 text-sm md:text-base">Evaluate your colleagues and provide valuable feedback</p>
            </div>

            <?php if (!$selected_colleague): ?>
                <!-- Colleague Selection View -->
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 md:gap-6">
                    <!-- Teacher Info Card -->
                    <div class="bg-gray-100 rounded-lg p-4 md:p-6 shadow-sm">
                        <h2 class="text-lg font-semibold text-gray-800 mb-5">Teacher Information</h2>
                        <div class="space-y-4">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm md:text-sm text-gray-600">Department: <?php echo htmlspecialchars($department); ?></p>
                                </div>
                            </div>
                            <div class="bg-yellow-100 border-l-4 border-yellow-500 p-4 rounded">
                                <p class="text-sm md:text-sm text-yellow-700">
                                    <strong>Instructions:</strong> Select a colleague from your department to begin the evaluation process.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-4 md:p-6 max-h-80 md:max-h-96 lg:max-h-[32rem] flex flex-col">
                        <h2 class="text-lg font-semibold text-gray-900 mb-5 flex-shrink-0">Select Colleague to Evaluate</h2>
                        ...
                        <div class="flex-1 overflow-y-auto">
                            <?php if (empty($colleagues)): ?>
                                <div class="text-center py-8 md:py-8 text-gray-500">
                                    ...
                                    <p class="text-sm md:text-base">No colleagues in your department yet.</p>
                                    <p class="text-xs md:text-sm text-gray-400 mt-1">Contact your administrator if this seems incorrect.</p>
                                </div>
                            <?php else: ?>
                                <div class="space-y-4">
                                    <?php foreach ($colleagues as $colleague): ?>
                                        <div class="border border-gray-200 rounded-lg p-4 md:p-4 hover:bg-gray-50 transition-colors">
                                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                                                <div class="flex-1">
                                                    <h3 class="font-medium text-gray-900 text-sm md:text-base"><?php echo htmlspecialchars($colleague['firstname'] . ' ' . $colleague['lastname']); ?></h3>
                    ...
                    <div class="max-w-4xl mx-auto">
                        <div class="bg-white rounded-lg shadow-md p-6 md:p-8">
                            ...
                            <div class="text-center mb-6">
                                ...
                                <p class="text-gray-600">You have successfully evaluated this colleague.</p>
                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-600">Department Colleague</p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-3">
                                        <p class="text-sm text-blue-700">
                                            <strong>Note:</strong> You can only evaluate each colleague once. Your feedback has been recorded and will be used for professional development purposes.
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-4 gap-1 md:gap-6 px-1 md:px-0">
                        ...
                        <div class="xl:col-span-1 bg-gray-100 rounded-lg p-3 md:p-6 shadow-sm mb-2 xl:mb-0">
                            ...
                            <div class="space-y-4">
                                <div class="bg-white rounded-lg p-4 md:p-4">
                                    <h3 class="font-medium text-gray-900 mb-3">Colleague</h3>
                                    <div class="flex items-center space-x-3">
                                        <div class="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center">
                                            <span class="text-white font-bold text-sm"><?php echo strtoupper(substr($selected_colleague['firstname'], 0, 1)); ?></span>
                                        </div>
                                        <div>
                                            <p class="font-semibold text-gray-900 text-sm md:text-base"><?php echo htmlspecialchars($selected_colleague['firstname'] . ' ' . $selected_colleague['lastname']); ?></p>
                                            <p class="text-xs md:text-sm text-gray-600"><?php echo htmlspecialchars($selected_colleague['position'] ?? 'Teacher') . ' • ' . htmlspecialchars($selected_colleague['department']); ?></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="bg-white rounded-lg p-4 md:p-4">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-xs md:text-sm text-gray-600"><?php echo htmlspecialchars($department); ?> Department</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="xl:col-span-3 bg-white rounded-lg shadow-md p-1 md:p-5 w-full min-w-0 evaluation-form-container">
                            <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-3 md:mb-6 space-y-3 md:space-y-0 w-full px-1 md:px-0">
                                <h2 class="text-lg md:text-xl font-semibold text-gray-900">Peer Evaluation Form</h2>
                            ...
                            <form id="evaluationForm" class="space-y-5 md:space-y-6 w-full mobile-form-wide">
                                <input type="hidden" name="teacher_id" value="<?php echo $selected_colleague['id']; ?>">
                                <input type="hidden" name="evaluator_type" value="teacher">

                                <div class="max-h-[60vh] md:max-h-[59vh] overflow-y-auto no-scroll w-full mobile-form-wide">
                                    ...
                                    <?php else: ?>
                                        <div class="text-center py-8 md:py-12">
                                            <svg class="w-12 md:w-16 h-12 md:h-16 text-gray-400 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                            </svg>
                                            <h3 class="text-lg md:text-xl font-semibold text-gray-900 mb-2">No Questions Available</h3>
                                            <p class="text-sm md:text-base text-gray-600">There are no evaluation questions configured for peer evaluation.</p>
                                            <p class="text-xs md:text-sm text-gray-500 mt-2">Please contact your administrator to set up evaluation criteria.</p>
                                        </div>
                                    <?php endif; ?>
                                </div>
...
<script>
    ...
    document.getElementById('evaluationForm').addEventListener('submit', function(e) {
        ...
        fetch('../actions/SubmitPeerEvaluation.php', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = '?module=peer_evaluation';
                    }, 1000);
                } else {
            ...
            .catch(error => {
                ...
                showToast('An error occurred while submitting the evaluation.', 'error');
            });
    });
    ...
    document.addEventListener('change', function(e) {
        if (e.target.type === 'radio') {
            ...
            const radioContainers = document.querySelectorAll(`input[name="${name}"] + div`);

            radioContainers.forEach(container => {
                container.classList.remove('border-blue-500', 'bg-blue-50', 'text-blue-700');
                container.classList.add('border-gray-300');
            });

            const selectedContainer = e.target.nextElementSibling;
            selectedContainer.classList.remove('border-gray-300');
            selectedContainer.classList.add('border-blue-500', 'bg-blue-50', 'text-blue-700');
        }
    });
</script>